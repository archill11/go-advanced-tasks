...
func downloadFile(w io.Writer, uri string) error {
	...
	if resp.StatusCode != http.StatusOK { // проверяем статус
...
// если recLevel = -1 - происходит скачивание до конца.
...
// copyWithRenewedLinks копирует данные from reader to writer.
// все найденные ссылки возвращаются в виде массива, а в копии - заменяются на пути к локальным файлам (если уровень рекурсии не 1).
func copyWithRenewedLinks(body io.Reader, file io.Writer, recLevel int) ([]string, error) {
	...
	var ok bool
	for {
		link, before, buf, ok = nextLink(buf) // находим ссылку
		if !ok {// если не нашли - записываем остаток
...
func nextLink(p []byte) (link string, before, after []byte, ok bool) {
	...
	if (hrefIdx != -1 && hrefIdx < srcIdx) || srcIdx == -1 {
		link, after, ok := getLink(s[hrefIdx+len(href):]) // достаём выражение в кавычках
		if !ok {
			return "", nil, p, false
		}
...
func main() {
	...
	filename := *outFile
	if filename == "" {
		filename = fileNameFromURI(uri) // либо генерируем из URL
