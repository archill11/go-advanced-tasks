...
// Ключом является первое слово из множетсва данных анаграмм, встретившееся в переданном списке.
// Значением является отсортированный массив анаграмм к слову-ключу.
...
// letterKeyMap - структура данных для определения ключа, инвариантного для каждого набора анаграмм.
// Ключом является строка вида "<буква1><число букв1>..."
...
func (s letterKeySlice) Len() int {
	return len(s)
}
func (s letterKeySlice) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}
func (s letterKeySlice) Less(i, j int) bool {
	return s[i][0] < s[j][0]
}

// String возвращает строку с отсортированными в алфавитном порядке парами буква-число букв.
